...
const getVoteValue = (type: "up" | "down" | "downPrevious" | "upPrevious", username: string, def: number): number => {
...
export class VoteDialog extends Component<VoteDialogProps, VoteDialogState> {
  ...
  upSliderChanged = (e: React.ChangeEvent<typeof FormControl & HTMLInputElement>) => {
    ...
    this.setState({
      ...
      showWarning:
        (upSliderVal < initialVoteValues.up || upSliderVal > initialVoteValues.up) && upSliderVal > 0 && upVoted,
    });
  };
        ...
        {mode === "up" && (
          <>
            <div className="voting-controls voting-controls-up">
              <div className="btn-vote btn-up-vote vote-btn-lg primary-btn-vote" onClick={this.upVoteClicked}>
        ...
        {mode === "down" && (
          <>
            <div className="voting-controls voting-controls-down">
              ...
              <div className="btn-vote btn-down-vote vote-btn-lg secondary-btn-vote" onClick={this.downVoteClicked}>
                <span className="btn-inner">{chevronDownSvgForSlider}</span>
              </div>
            </div>
...
export class EntryVoteBtn extends BaseComponent<Props, State> {
  ...
  render() {
    ...
    if (upVoted || downVoted) {
      cls = _c(
        `btn-vote ${upVoted ? "btn-up-vote primary-btn-done" : "btn-down-vote secondary-btn-done"} ${
    ...
    const voteBtnClass = `btn-inner ${
      tooltipClass.length > 0 ? (upVoted ? "primary-btn-done" : downVoted ? "secondary-btn-done" : "primary-btn") : ""
    }`;
