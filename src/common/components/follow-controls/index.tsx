...
export default class FollowControls extends BaseComponent<Props, State> {
  ...
  fetch = async () => {
    ...
    try {
      const [followingInfo, mutedInfo] = await Promise.all([
        getFollowing(activeUser.username, targetUsername, "blog", 1),
        getFollowing(activeUser.username, targetUsername, "ignore", 1),
      ]);

      let isFollowing = false;
      let isMuted = false;

      if (followingInfo.length > 0 && followingInfo[0].following == targetUsername) {
        isFollowing = true;
      }

      if (mutedInfo.length > 0 && mutedInfo[0].following == targetUsername) {
        isMuted = true;
      }

      this.stateSet({ following: isFollowing, muted: isMuted });
    } catch (e) {
    } finally {
      this.stateSet({ fetching: false });
    }
  ...
  render() {
    ...
    const btnFollow = LoginRequired({
      ...this.props,
      children: (
        <Button variant="primary" style={{ marginRight: "5px" }} disabled={inProgress} onClick={this.follow}>
          {followMsg}
        </Button>
      ),
    });
    ...
    const btnUnfollow = LoginRequired({
      ...this.props,
      children: (
        <Button variant="primary" style={{ marginRight: "5px" }} disabled={inProgress} onClick={this.unFollow}>
          {unFollowMsg}
        </Button>
      ),
    });
