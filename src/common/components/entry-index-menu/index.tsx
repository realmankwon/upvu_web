...
export class EntryIndexMenu extends Component<Props, States> {
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    // }
      ...
      const tagValue =
        tag && tag !== "my" && ["today", "week", "month", "year", "all"].includes(tag) ? "/" + tag : "/today";
  ...
  render() {
    ...
    const introductionOverlayClass =
      (isMounted && (introduction === IntroductionType.NONE ? "d-none" : "overlay-for-introduction")) || "d-none";
    return isMounted ? (
      <div>
                    ...
                    {mobileMenuConfig.items.map((i, k) => {
                      return (
                        <li key={k} className="nav-item">
                          <Link to={i.href!} className={_c(`nav-link link-${i.id} ${i.selected ? "active" : ""}`)}>
                            {i.label}
                          </Link>
                        </li>
                      );
