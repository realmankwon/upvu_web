...
import { lookupAccounts } from "../../api/steem";
...
const AddColumn = ({ setSelectedValue, onSelect, selectedValue, decks }: any) => {
      ...
      return (fetchData as any)(searchTerm, 5, toValue)
        .then((resp: any) => {
          if (resp) {
            ...
            if (existing) {
              let valueToSelect = selectedValue === _t("decks.community") ? existing.name : existing;
  ...
  const suggestionProps = {
    ...
    onSelect: (selectedText: any) => {
      let valueToSelect = selectedValue === _t("decks.community") ? selectedText.name : selectedText;
  ...
  const handleAddColumn = () => {
    let couldBeExistingDeck = `${
      selectedValue === _t("decks.users") || selectedValue === _t("decks.community") ? contentType : selectedValue
  ...
  return (
    <div className="d-flex flex-column align-items-center mt-5">
      ...
      <div className="my-5 d-flex align-items-center back-icon pointer w-100" onClick={() => setSelectedValue(null)}>
...
export const DeckAddModal = ({ open, onClose, onSelect, currentlyActivatedOptions, activeUser }: any) => {
        ...
        <div className="flex-grow-1 text-center title">
          ...
          (selectedOption === _t("decks.users") ||
            ...
            <div className="d-flex align-items-center justify-content-center">
              ...
              <div>
                {_t("decks.add")} {options(activeUser).find((item) => item.title === selectedOption)?.title}{" "}
                {_t("decks.column")}
              </div>
            </div>
          ) : (
