...
const Index = (props: PageProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      props.setLastIndexPath(showLandingPage ? "feed" : "feed");
    };
  ...
  const initStep = (nextStep?: number) => {
    ...
    const nextShowLandingPage =
      currentStep === 1 && props.activeUser === null && props.location && "/" === props.location?.pathname;
  ...
  return (
    <>
      ...
      {showLandingPage && <EntryIndexContainer {...props} loading={loading} setLoading={setLoading} reload={reload} />}
      {showEntryPage && <EntryIndexContainer {...props} loading={loading} setLoading={setLoading} reload={reload} />}
    </>
  );
};
