...
class CommunityCreatePage extends BaseComponent<PageProps, CreateState> {
  ...
  makeAuthorities = (keys: { ownerKey: PrivateKey; activeKey: PrivateKey; postingKey: PrivateKey }) => {
    ...
    return {
      ...
      postingAuthority: Authority.from(postingKey.createPublic()) as Authority,
    };
  };
  ...
  submit = async () => {
    ...
    return this.finalizeSubmit(code, access_token, refresh_token);
  };
  ...
  finalizeSubmit = async (hsCode: string, access_token: string | undefined, refresh_token: string | undefined) => {
    ...
    let user: User;
    try {
      user = await hsTokenRenew(hsCode, access_token, refresh_token).then((x) => ({
        username: x.username,
                                ...
                                <Form.Group>
                                  <div className="d-flex align-items-center">
                                    ...
                                    <OverlayTrigger
                                      placement={"bottom"}
                                      overlay={
                                        <Tooltip id={`tooltip-bottom`}>{_t("communities-create.reason-four")}</Tooltip>
                                      }
                                    >
                                ...
                                <Form.Group>
                                  ...
                                  {usernameStatus === "conflict" && (
                                    <Form.Text className="text-danger">
                                      {alertCircleSvg} {_t("communities-create.username-not-available")}
                                    </Form.Text>
                                  )}
                                  {usernameStatus === "not-valid" && (
                                    <Form.Text className="text-danger">
                                      {alertCircleSvg} {_t("communities-create.username-wrong-format")}
                                    </Form.Text>
                                  )}
                                </Form.Group>
                                <Form.Group>
                                  ...
                                  <Form.Group>
                                    <InputGroup className="mb-3" onClick={() => this.copyToClipboard(wif)}>
                                      <Form.Control
                                        value={wif}
                                        disabled={true}
                                        className="pointer"
