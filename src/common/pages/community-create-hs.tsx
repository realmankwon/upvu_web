...
class CommunityCreateHSPage extends BaseComponent<PageProps, CreateHsState> {
  ...
  handle = async () => {
    ...
    let user: User;
    try {
      const access_token = getAccessToken(activeUser.username);
      const refresh_token = getRefreshToken(activeUser.username);

      user = await hsTokenRenew(code, access_token, refresh_token).then((x) => ({
        username: x.username,
