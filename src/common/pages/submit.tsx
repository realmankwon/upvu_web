...
class SubmitPage extends BaseComponent<Props, State> {
  ...
  publish = async (): Promise<void> => {
    ...
    const { title, tags, body, description, reward, reblogSwitch, beneficiaries, selectedThumbnail, selectionTouched } =
      this.state;
              ...
              <div className="community-input">
                {CommunitySelector({
                  ...
                  onSelect: (prev, next) => {
                    ...
                    const newTags = [...[next ? next : ""], ...tags.filter((x) => x !== prev)].filter((x) => x);
            ...
            const toolBar = schedule ? (
              <div className="bottom-toolbar">
                ...
                {LoginRequired({
                  ...this.props,
                  children: (
                    <Button className="d-inline-flex align-items-center" onClick={this.schedule} disabled={posting}>
                      {posting && spinner}
                      {_t("submit.schedule")}
                    </Button>
                  ),
                ...
                {editingEntry !== null && (
                  <>
                    ...
                    {LoginRequired({
                      ...this.props,
                      children: (
                        <Button className="d-inline-flex align-items-center" onClick={this.update} disabled={posting}>
                          {posting && spinner}
                          {_t("submit.update")}
                        </Button>
                      ),
