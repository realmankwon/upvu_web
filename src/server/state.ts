...
export const makePreloadedState = async (req: express.Request): Promise<AppState> => {
  ...
  const listStyle =
    _c("list-style") && Object.values(ListStyle).includes(_c("list-style")) ? _c("list-style") : defaults.listStyle;
  ...
  const globalState: Global = {
    ...
    isMobile: !!(
      req.headers["user-agent"] && ["iOS", "AndroidOS"].includes(getOperatingSystem(req.headers["user-agent"]))
    ),
    usePrivate: Boolean(parseInt(config.usePrivate, 10)),
    developingPrivate: Boolean(parseInt(config.developingPrivate, 10)),
  };
